...
const navigate = useNavigate();
  ...
  return (
    ...
    <div className="flex flex-col justify-between items-center gap-5 md:flex-row md:gap-0">
        <button className="border rounded-full py-2 border-slate-200 px-5 flex items-center justify-center gap-2 text-xs font-medium uppercase tracking-widest hover:bg-stone-100 transition-all" onClick={()=>navigate(-1)}>
        ...
        <div className="flex flex-col items-center gap-1 5">
          <h1 className="text-3xl font-bold">Pedido #{id}</h1>
            <p className="text-sm">{formateDatelong(order.create_at)}</p>
        </div>
          <div></div>
          <div></div>
    </div>
    ...
    <div className="flex flex-col mt-10 mb-5 gap-10">
        <table className="text-sm w-full caption-bottom overflow-auto">
            <thead>
                <tr>
                  {tableHeaders.map((header, index) =>(
                        <th key={index} className="h-12 text-center uppercase tracking-wide text-stone-600 font-medium">{header}
                        </th>
                  ))}
                </tr>
            </thead>
            <tbody>
                  {order.orderItems.map((product,index)=>(
                    <tr key={index} className="borde-b border-gray-200">
                        <td className="p-4 font-medium tracking-tighter flex gap-3 items-center">
                            <img src={product.productImages} alt={product.productName} className="h-20 w-20 object-contain rounded-lg"/>
        ...
        <div className="flex flex-col gap-3">
              ...
              <div className="border border-stone-300 p-5 flex flex-col gap3">
